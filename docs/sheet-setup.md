...
tab itself the first time any write happens, with a fixed header row:

| Timestamp | Actor | Action | Entity | Summary | Changes |
| --- | --- | --- | --- | --- | --- |
| ISO 8601 UTC | participant name | `add` / `update` / `delete` | `expense` / `trip` / `category` | one-line label for the item | field-by-field diff for edits, e.g. `amount: 50 → 84.5`; empty otherwise |
...
`getTripSnapshot_` and the `format*Summary`/`diff*` functions in `sheet.js`.
Nothing is ever deleted from this tab by the app; there's no size cap (a
household's write volume would take decades to meaningfully dent Sheets'
10-million-cell ceiling, shared across every tab in the file).
